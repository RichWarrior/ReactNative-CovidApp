...
class Daily extends React.Component {
  ...
  componentDidMount = () => {
    var req = {
      "query": "query{getDataMonthly{date dailyTest dailyCase dailySick dailyDeath dailyHealing totalTest totalSick totalDeath}}",
      "variables": {
      }
    }
    ...
    httpClient.Post("/api/Covid", req).then((res) => {
      this.setState({ data: res.data.getDataMonthly })
    }).catch(() => {
      alert('Sunucu Hatası')
    }).finally(() => {
  ...
  onButtonClick = () => {
    var req = {
      "query": "query($startDate : Date!,$endDate : Date!){getDataMonthlyBetweenDates(startDate : $startDate,endDate : $endDate){date dailyTest dailyCase dailySick dailyDeath dailyHealing totalTest totalSick totalDeath}}",
      "variables": {
    ...
    httpClient.Post("/api/Covid", req).then((res) => {
      ...
      this.setState({ data: res.data.getDataMonthlyBetweenDates })
    }).catch(() => {
      alert('Sunucu Hatası')
    }).finally(() => {
      this.setState({ onRefresh: false })
