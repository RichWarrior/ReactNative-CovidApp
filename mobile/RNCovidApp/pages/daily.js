...
class Daily extends React.Component {
  ...
  componentDidMount = () => {
    var req = {
      "query": "query{getDataDaily{date dailyTest dailyCase dailySick dailyDeath dailyHealing totalTest totalSick totalDeath}}",
      "variables": {
      }
    }
    ...
    httpClient.Post("/api/Covid", req).then((res) => {
      console.clear();
      console.log(res)
      this.setState({ data: res.data.getDataDaily })
    }).catch(() => {
      alert('Sunucu Hatası')
    }).finally(() => {
  ...
  onButtonClick = () => {
    var req = {
      "query": "query($startDate : Date!,$endDate : Date!){getDataBetweenDatesDaily(startDate : $startDate,endDate : $endDate){date dailyTest dailyCase dailySick dailyDeath dailyHealing totalTest totalSick totalDeath}}",
      "variables": {
    ...
    httpClient.Post("/api/Covid", req).then((res) => {
      console.clear();
      console.log(res)
      this.setState({ data: res.data.getDataBetweenDatesDaily })
    }).catch(() => {
      alert('Sunucu Hatası')
    }).finally(() => {
      this.setState({ onRefresh: false })
